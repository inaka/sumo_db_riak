...
-export([find_all/1, find_by/1, update/1, delete_all/1, delete/1]).
...
-spec all() -> [atom()].
all() -> [find_all, find_by, update, delete_all, delete].
...
find_by(_Config) ->
  ...
  [
    ...
    lists:sort(fun (A, B) -> maps:get(id, A) =< maps:get(id, B) end, Results1),
  ...
  Results3 = sumo:find_by(purchases, [{'ship_to.city', <<"city2">>}]),
  1 = length(Results3),
  Results2 = Results3,
  Results4 =
    sumo:find_by(
...
init_store() ->
  ...
  sync_timeout(0),
  Addr1 =
    sumo_test_purchase_order:new_address(
      <<"l1">>,
      <<"l2">>,
      <<"city1">>,
      <<"s">>,
  ...
  lists:foreach(
    fun (Doc) -> sumo:persist(purchases, Doc) end,
